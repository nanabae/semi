...
public class DramaDao {
	...
	public List<DramaVo> selectDramaList(Connection conn, PageVo pv, String catNum, String searchType,String searchValue , String headerNum) throws Exception {
		String sql = "SELECT * FROM ( SELECT ROWNUM RNUM , T.* FROM ( SELECT D.DRAMA_BRD_NUM , D.CAT_NUM ,H.HEADER_NUM,H.HEADER_NAME ,D.TITLE , D.CONTENT , D.DRAMA_WRITER ,  D.ENROLL_DATE , D.STATUS , D.MODIFY_DATE , D.HIT ,D.ANONYMITY, M.MEM_NICK ,C.CAT_NAME FROM DRAMA_BOARD D JOIN MEMBER M ON(D.DRAMA_WRITER = M.MEM_NUM) JOIN CATEGORY C ON (D.CAT_NUM = C.CAT_NUM)";
		//카테고리 번호,말머리 번호 유무
		if(headerNum.equals("") && catNum.equals("")) {
			sql +="LEFT OUTER JOIN HEADER H ON (D.HEADER_NUM = H.HEADER_NUM) WHERE D.STATUS = 'O'";
			
		}else if( catNum.equals("") && !headerNum.equals("")){
			sql +="JOIN HEADER H ON ( D.HEADER_NUM = H.HEADER_NUM) WHERE D.STATUS = 'O' AND D.HEADER_NUM = " + headerNum +"";
			
		}else if(headerNum.equals("") && !catNum.equals("")) {
			sql +="LEFT OUTER JOIN HEADER H ON ( D.HEADER_NUM = H.HEADER_NUM) WHERE D.STATUS = 'O' AND C.CAT_NUM = " + catNum +"";
		}else {
			sql +="JOIN HEADER H ON ( D.HEADER_NUM = H.HEADER_NUM) WHERE D.STATUS = 'O' AND C.CAT_NUM = " + catNum + "AND D.HEADER_NUM = " + headerNum +"";
